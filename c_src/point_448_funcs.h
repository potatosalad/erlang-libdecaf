...
static ERL_NIF_TERM
libdecaf_x448_generate_key_1(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	ErlNifBinary scalar;

	if (argc != 1 || !enif_inspect_binary(env, argv[0], &scalar)
			|| scalar.size != DECAF_X448_PRIVATE_BYTES) {
	...
	(void) decaf_x448_generate_key(u, scalar.data);

	return out;
}

static ERL_NIF_TERM
libdecaf_x448_2(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	ErlNifBinary base;
	ErlNifBinary scalar;

	if (argc != 2 || !enif_inspect_binary(env, argv[0], &base)
			|| base.size != DECAF_X448_PUBLIC_BYTES
