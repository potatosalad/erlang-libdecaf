...
init([]) ->
	...
	case load() of
		ok ->
			Port = erlang:open_port({spawn_driver, ?ED448_DRIVER_NAME}, [binary]),
			erlang:register(?ED448_DRIVER_ATOM, Port),
		...
		{error, LoadError} ->
			...
			ErrorStr = lists:flatten(io_lib:format(
				...
				[?ED448_DRIVER_NAME, LoadErrorStr])),
			{stop, ErrorStr}
	end.
...
terminate(_Reason, #state{port=Port}) ->
	erlang:unregister(?ED448_DRIVER_ATOM),
...
load() ->
	...
	case lists:member(?ED448_DRIVER_NAME, Drivers) of
		true ->
			ok;
		false ->
			case erl_ddll:load(priv_dir(), ?ED448_DRIVER_NAME) of
				ok ->
					ok;
				...
				{error, Error} ->
					error_logger:error_msg(
						...
						[?ED448_DRIVER_NAME, erl_ddll:format_error(Error)]
					),
					{error, Error}
			end
	end.
...
%%--------------------------------------------------------------------
unload() ->
	case erl_ddll:unload_driver(?ED448_DRIVER_NAME) of
		ok ->
			ok;
		{error, Error} ->
			error_logger:error_msg(
